...
interpretation is essential to assure reproducibility of an analysis
pipeline and to comprehensively communicate these results to
...
`shiny` in an R package that allows to build interactive web
applications from R. These apps are composed by a user interface part
and a server backend. Here, we will save each of these in the `ui.R`
and `server.R` files respectively. Both files are stored into a
directory, `app1` below. Both sides continuously react to their
respective updates, hence the term of *reactive programming*.

## Server

The server backend defines the server functionality through the
`shinyServer` function, which itself takes an anyonymous function as
...
display the value `"hist"`, that is returned by the server backend.
...
the directory contain the server and interface definitions. 
...
Let's try to improve our simple first app by adding. A suggestion if
available by running the `app2()` function. The additional widgets
added to the interface are `radioButtons`, a `numericInput` field
(side bar) and a `verbatimTextOutput` (main panel). The server backend
takes advantage if two new inputs to add a vertical line to the
histogram and add a table of significant/non-significant features to
its output.

A few useful links : 
- [`shiny` tutorial](http://shiny.rstudio.com/tutorial/)
- List of available
[widgets](http://shiny.rstudio.com/gallery/widgets-gallery.html).
